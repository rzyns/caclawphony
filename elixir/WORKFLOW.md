...
- `land`: when ticket reaches `Merging`, apply `land` in a loop until merged.
...
- `Merging` -> approved by human; merge flow in progress.
...
3. Route to the matching flow:
   ...
   - `Merging` -> apply `land` loop until merged.
...
5. When the issue is in `Merging`, run the `land` skill in a loop until the PR is merged.
